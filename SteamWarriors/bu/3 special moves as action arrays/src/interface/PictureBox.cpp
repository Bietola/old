...
///constructors
//normal constructors
PictureBox::PictureBox(const sf::FloatRect& bounds, const sf::Texture& texture, const sf::IntRect& textureRect):
    Interface(bounds),
...
PictureBox::PictureBox(const sf::FloatRect& bounds, const sf::Texture& texture):
    PictureBox(bounds, texture, sf::IntRect(0, 0, texture.getSize().x, texture.getSize().y))
{}
///setters
//set bounds
void PictureBox::setBounds(const sf::FloatRect& bounds){
    if(getBounds() != bounds){
        //set actual bounds
...
void PictureBox::setTextureRect(const sf::IntRect& textureRect){
...
///graphic functions
void PictureBox::parentDraw(sf::RenderTarget& target, sf::RenderStates states) const {
