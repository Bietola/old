    ...
    mAnimationHandler()
{
    ...
    ///set up interface
    //tile info interface
    mTileInfo = new gr::Interface(sf::FloatRect(world->tileToWindowPosition(position).x, world->tileToWindowPosition(position).y, TILE_WIDTH, TILE_HEIGHT));
    mTileInfo->addRelative<gr::Bar>("life_bar", sf::FloatRect(1.f/15.f, 14.f/15.f, 13.f/15.f, 1.f/15.f), [this](){return mStatus.get("life");}, mStatus.get("max-life"), 0, sf::Color::Green);
}
