...
void get_players(){
	int j=0;
    plNum=5;
	players=(struct Player*)realloc(players,sizeof(struct Player)*plNum);
	for(j=0;j<plNum;j++){
		players[j].state=FREE;
	}
	/*strcpy("stefano",players[0].name);		players[0].role=USER;
	strcpy("chicco",players[1].name);		players[1].role=USER;
	strcpy("tina",players[2].name);			players[2].role=CPU;
	strcpy("daniele",players[0].name);		players[0].role=CPU;
	strcpy("stocazzone",players[0].name);	players[0].role=CPU;*/
	players[0].name="stefano";      players[0].role=USER;
	players[1].name="chicco";       players[1].role=USER;
	players[2].name="tina";         players[2].role=CPU;
	players[3].name="daniele";      players[3].role=CPU;
	players[4].name="stocazzone";   players[4].role=CPU;
