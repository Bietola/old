...
float risolvi_proc(char* proc,int mod){
    ...
    if(totCoef==0) err=ERR_IND;                 //equazione indeterminata
    if(modCoef==2 && totTerNot==0) err=ERR_IMP; //divisione per zero
    if(parte==1) err=ERR_INV;                   //stringa invalida perche' senza uguale
    //restituzione e stampo (se necessario) di eventuali errori avvenuti durante la risoluzione
    if(mod-PROC_ERR_PRINT>=0){
        if(err==ERR_IND) printf("ERR: equazione indeterminata\n");
        else if(err==ERR_IMP) printf("ERR: equazione e' impossibile (divisione per 0)\n");
        else if(err==ERR_INV) printf("ERR: stringa di procedimento invalida\n");
        mod-=PROC_ERR_PRINT;
    }
    if(err!=0) return err;
    ...
    //calcolo e restituzione del risultato della stringa di procedimento (calcolo dipende dalla modalita' del coefficiente)
